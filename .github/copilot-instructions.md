...
- **Portfolio analytics block** (`R/mod_dashboard.R` + `R/portfolio_analytics.R`): the dashboard now has a 5Y `PORTFOLIO PERFORMANCE` chart plus an on-demand `CORRELATION & DIVERSIFICATION` section below the charts. The performance chart backcasts the current holdings mix and can overlay optional benchmarks `MSCI World` (Yahoo proxy `URTH`) or `S&P 500` (`SPY`). The analytics section computes EUR-based daily return correlations for held positions, portfolio-level benchmark links (Gold, S&P 500, Nasdaq, Treasuries, defensive sleeves, etc.), CAPM beta and ETF factor proxies, a volatility-reduction candidate table for hypothetical add-on buys, and a similar-stock screen for direct holdings. Keep it Yahoo-only; do not spend Alpha Vantage calls for this feature.
...
- **Price + FX data** (`R/market_data.R`): use Yahoo Finance `v8/finance/chart` directly; do not use `quantmod::getQuote()` because it fails in Germany due to Yahoo GDPR crumb requirements. Quotes are fetched in parallel, cached for 30 seconds in `.quote_cache`, and reused across dashboard/portfolio/watchlist. EUR conversion uses Yahoo FX pairs like `USDEUR=X` and `SGDEUR=X`. Historical series are also fetched directly from the same endpoint with adjusted-close support and cached in `.history_cache` by ticker + lookback + field. When parsing chart history JSON, preserve `NULL` gaps in Yahoo close arrays; simple `unlist()` breaks timestamp alignment, especially for FX histories.
...
- **Portfolio candidate universe** (`data/sp500_tickers.csv`): the correlation/diversification scan uses this local S&P 500 ticker list as its stock universe for both similar-name matching and low-correlation candidate screening. Treat it as the default equity universe unless the user explicitly asks for a broader one.
- **Data sources**: Prices + FX = Yahoo Finance v8 chart API (free, unlimited). Portfolio correlation / factor analytics = Yahoo adjusted daily history + Yahoo FX history + local `data/sp500_tickers.csv`. Fundamentals = persistent cache first, then Alpha Vantage overview-first (key required, 25/day free), SEC EDGAR first for US statement data, Alpha Vantage statements mainly for international or missing SEC coverage, and Yahoo Finance as the broader fallback. ETF holdings = iShares CSV + justETF.com.
